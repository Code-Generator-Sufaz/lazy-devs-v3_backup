        ...
        User.findOne({ githubID: profile._json.id }, (err, user) => {
            ...
            if (profile._json.email) {
                User.findOne({ email: profile._json.email }, (err, existUser) => {
                    ...
                    if (existUser) // if we find this email, update this user with his github id
                    ...
                    else { // no existUser we will save him
            ...
            } else {  // profile._json.email is not true
    ...
    passport.use(new GoogleStrategy({
        ...
        function (accessToken, refreshToken, profile, cb) {
            ...
            User.findOne({ googleID: profile._json.sub }, (err, user) => {
                ...
                const googleEmail = 'thisIsFakeEmail' + profile._json.sub + '@google.com';
                // here need bcrypt the password
                bcrypt.hash(profile._json.picture, 10, (err, notPassword) => {
                    ...
                    User.create({
                        ...
                        .then(newUser => { return cb(null, newUser) })
                        .catch(err => console.error(err))
                })
            });
        }));
